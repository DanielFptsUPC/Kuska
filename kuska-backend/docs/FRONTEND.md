...
Los reportes que conservan fotos o video locales se reintentan individualmente mediante
`POST /incidents` multipart, siempre con el mismo `client_id`. `/sync/batch` solo admite
metadatos o URLs HTTP de evidencia ya subida; no enviar rutas `local://` ni `file://`.
...
3. Servicio de sync: cuando detecte conexión (`@react-native-community/netinfo`), sube cada
   reporte con evidencia local vía `POST /incidents` multipart usando su `client_id`; usa
   `/sync/batch` para reportes sin archivos locales o con evidencia ya subida, y marca como
   sincronizados los que el backend confirme.
