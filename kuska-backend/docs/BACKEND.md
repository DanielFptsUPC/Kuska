...
POST /sync/batch
  application/json:
    { "incidents": [
        { client_id, description, lat, lon, created_at_client,
          photo_urls: ["https://..."], video_url: "https://..." | null }
      ]
    }
  → 200 [{ client_id, incident_id, status, created }]
...
### Contrato definitivo de evidencia offline

- Si el reporte tiene fotos o video todavía guardados en el teléfono, el móvil lo envía
  individualmente a `POST /incidents` como multipart. En cada reintento conserva el mismo
  `client_id`; el backend devuelve el mismo `incident_id` y no duplica el reporte.
- `/sync/batch` se reserva para metadatos o archivos que ya fueron subidos y tienen una URL
  `http://` o `https://` accesible por el backend.
- Nunca se envían referencias `local://` o `file://`: esas rutas solo existen en el dispositivo.
- El wrapper `{ "incidents": [...] }` es parte estable del contrato y permite añadir metadatos
  del lote en el futuro sin cambiar el tipo raíz de la petición.
